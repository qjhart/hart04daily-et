...
#
# This writes the CIMIS station file
#
function cimis.pg.to.station { 
    eval `g.gisenv | sed -e 's/^/local /'`
    local sdir=$GISDBASE/$LOCATION_NAME/$MAPSET/site_lists
    [ -d $sdir ] || mkdir $sdir;
    if [ -z $1 ]; then
    ...
    psql -A -F'|' -t -c "select x(st_casil83),y(st_casil83),st_elevation*12*2.54/100,\
 '#'||st_number||' @\"'||st_name||'\" @\"'||st_city||'\" @'||st_county||' @'||st_start||' @'||CASE WHEN st_end is null THEN 'ACTIVE' ELSE st_end::text END  \
from station order by st_number;" -h $PG_HOST -d $PG_DBASE > $sdir/$station;
}
